...
pub type Connecting = Pin<Box<dyn Future<Output = Result<H3Connection, BoxError>> + Send>>;

pub trait H3Connector: std::fmt::Debug + Send + Sync {
    fn connect(&self, dest: Uri) -> Connecting;
}
...
impl DynH3Connector {
    ...
    pub fn connect(&self, dest: Uri) -> Connecting {
